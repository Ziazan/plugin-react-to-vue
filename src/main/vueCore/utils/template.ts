...
import isDOMEvent from './../helpers/is-dom-event';
...
export const genCommentLineAst = template(`
   // COMMENT
`);
...
export const transformVueTemplate = ({
  ...
  if (t.isJSXElement(node)) {
    ...
    //   });
    // }
    str += tools.toHorizontalLine(tagName);
      ...
      if (name === 'className') {
        ...
        if (t.isJSXExpressionContainer(value)) {
          ...
          if (hasStyles) {
            ...
            attrItemString += `:class="${tools.changeMemberExpressionByName(stringLiteral)}"`;
          }
        } else {
        ...
        attrItemString += `@${name}="${stringLiteral}"`;
      } else {
        //值绑定加冒号
        ...
        if (isDOMEvent(name)) {
          //DOM事件
          attrItemString += `@${name.substr(2).toLowerCase()}`;
        } else {
          attrItemString += name;
        }
        if (value) {
